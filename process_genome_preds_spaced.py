...
def process_predictions(arr):
  ...
  for window in arr2:
    ...
    sumcount += 1
    num_indels_pred = np.sum(window[window[:, 1]%small_window == small_window_sizewindow[:, 1]%small_window == small_window_size, 2] > thresh) # Predicted number of indels in the bucket, using thresholding
    sumprobs = np.sum(window[window[:, 1]%small_window == small_window_size, 2]) # Predicted number of indels in the bucket, using summing of probabilities
