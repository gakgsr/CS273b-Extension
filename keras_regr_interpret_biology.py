...
cdng = pd.read_csv(data_dir + "hinv70NonCoding.txt", sep = '\t', header = None)
...
over_pred, over_pred_cdng = 0, 0
undr_pred, undr_pred_cdng = 0, 0
c = ['b']*len(y_test)
for i in range(len(y_test)):
  if(y_pred[i] > 1.1*y_test[i]):
    over_pred += 1
  if(y_pred[i] > 1.1*y_test[i] and np.sum(cdng_regions[i*window_size + margin:(i+1)*window_size + margin]) > 0.5*window_size):
    over_pred_cdng += 1
    c[i] = 'y'
  if(y_pred[i] < 0.9*y_test[i]):
    undr_pred += 1
  if(y_pred[i] < 0.9*y_test[i] and np.sum(cdng_regions[i*window_size + margin:(i+1)*window_size + margin]) > 0.5*window_size):
    undr_pred_cdng += 1
    c[i] = 'r'
...
plt.savefig('indel_rate_pred_keras_non_coding_rgn' + str(validation_chrom) + str(complexity_thresh) + '_' + str(windows_per_bin) + '.png')
