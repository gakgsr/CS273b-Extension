...
np.save(data_dir + 'RegrKerasEntropyTestLab' + str(validation_chrom) + '.npy', y_test)
...
model = Sequential()
model.add(Dense(1, activation='linear', input_shape=(k + margin, )))
...
np.save(data_dir + 'RegrKerasEntropyTestLabPred' + str(validation_chrom) + '.npy', y_pred)
...
plt.savefig('indel_rate_pred_keras_entropy' + str(validation_chrom) + '.png')
